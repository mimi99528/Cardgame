...
class CardDisplay:
    ...
    def draw_hand(self, battle: BattleSystem):
        ...
        # 计算卡牌位置
        card_width = self.ui_renderer.ss(CONSTANTS.CARD_WIDTH)
        card_height = self.ui_renderer.ss(CONSTANTS.CARD_HEIGHT)
        spacing = self.ui_renderer.ss(CONSTANTS.CARD_SPACING)
        
        total_width = len(hand) * (card_width + spacing) - spacing
        start_x = (self.ui_renderer.ui_scale.window_width - total_width) / 2
        base_y = self.ui_renderer.sy(60)
        ...
        for i, card in enumerate(hand):
            ...
            if id(card) == id(self.hovered_card):
                draw_y += self.ui_renderer.ss(CONSTANTS.CARD_HOVER_OFFSET)
    ...
    def _draw_card(self, card: Card, x, y, width, height):
        ...
        if card_test_texture:
            ...
            arcade.draw_texture_rect(
                ...
                arcade.XYWH(
                    x - image_width / 2,
                    y + self.ui_renderer.ss(10) - image_height / 2,
                    image_width,
                    image_height
                )
            )
        ...
        # 卡牌名称背景
        name_bg_y_bottom = y - height / 2 + self.ui_renderer.ss(110)
        name_bg_y_top = y - height / 2 + self.ui_renderer.ss(150)
        arcade.draw_lrbt_rectangle_filled(
            x - self.ui_renderer.ss(100), x + self.ui_renderer.ss(100),
        ...
        arcade.draw_lrbt_rectangle_outline(
            x - self.ui_renderer.ss(100), x + self.ui_renderer.ss(100),
            name_bg_y_bottom, name_bg_y_top,
            arcade.color.WHITE, self.ui_renderer.ss(5)
        )
        
        # 卡牌名称
        self.ui_renderer.draw_text(
            card.name,
            x, name_bg_y_bottom + self.ui_renderer.ss(20),
            arcade.color.WHITE,
    ...
    def _draw_ap_cost(self, card: Card, x, y, width, height):
        """绘制AP消耗"""
        ap_x = x + width / 2 - self.ui_renderer.ss(15)
        ap_y = y + height / 2 - self.ui_renderer.ss(15)
        ...
        for i in range(card.ap_cost):
            if texture:
                arcade.draw_texture_rect(
                    texture,
                    arcade.XYWH(
                        ap_x - self.ui_renderer.ss(10),
                        ap_y - i * self.ui_renderer.ss(25) - self.ui_renderer.ss(10),
                        self.ui_renderer.ss(20),
                        self.ui_renderer.ss(20)
                    )
                )
            else:
                arcade.draw_circle_filled(ap_x, ap_y - i * self.ui_renderer.ss(25), self.ui_renderer.ss(8), arcade.color.BLUE)
                arcade.draw_circle_outline(ap_x, ap_y - i * self.ui_renderer.ss(25), self.ui_renderer.ss(8), arcade.color.WHITE, self.ui_renderer.ss(2))
    ...
    def _draw_card_tags(self, card: Card, x, y, width, height):
        ...
        tag_height = self.ui_renderer.ss(18)
        tag_spacing = self.ui_renderer.ss(5)
        ...
        for i, tag in enumerate(display_tags):
            ...
            # 计算标签位置
            tag_width = self.ui_renderer.ss(50)
            tag_x = x - width / 2 + self.ui_renderer.ss(10) + col * (tag_width + tag_spacing)
    ...
    def _draw_effect_values(self, card: Card, x, y, width, height):
        ...
        if "hp" in card.effects:
            ...
            icon_x = x - width / 2 + self.ui_renderer.ss(30)
            icon_y = y + height / 2 - self.ui_renderer.ss(30)
            
            # 白色背景框
            arcade.draw_lrbt_rectangle_filled(
                icon_x - self.ui_renderer.ss(27), icon_x + self.ui_renderer.ss(27),
                icon_y - self.ui_renderer.ss(15), icon_y + self.ui_renderer.ss(15),
                arcade.color.WHITE
            )
            arcade.draw_lrbt_rectangle_outline(
                icon_x - self.ui_renderer.ss(27), icon_x + self.ui_renderer.ss(27),
                icon_y - self.ui_renderer.ss(15), icon_y + self.ui_renderer.ss(15),
                arcade.color.BLACK, self.ui_renderer.ss(2)
            )
            ...
            if "phy" in card.card_type.value and phy_texture:
                arcade.draw_texture_rect(
                    phy_texture,
                    arcade.XYWH(
                        icon_x - self.ui_renderer.ss(20),
                        icon_y - self.ui_renderer.ss(10),
                        self.ui_renderer.ss(20),
                        self.ui_renderer.ss(20)
                    )
                )
            ...
            self.ui_renderer.draw_text(
                str(value),
                icon_x + self.ui_renderer.ss(10), icon_y,
                arcade.color.BLACK,
    ...
    def _draw_card_description(self, card: Card, x, y, width, height):
        ...
        # 绘制多行描述
        line_height = self.ui_renderer.ss(18)
        max_line_width = width - self.ui_renderer.ss(20)  # 留出边距
        
        # 计算总高度
        total_height = len(desc_lines) * line_height + self.ui_renderer.ss(10)
        
        # 背景框
        arcade.draw_lrbt_rectangle_filled(
            x - width / 2 - self.ui_renderer.ss(5), x + width / 2 + self.ui_renderer.ss(5),
        ...
        for i, line in enumerate(desc_lines):
            line_y = description_y + total_height - self.ui_renderer.ss(10) - i * line_height
    ...
    def _draw_ai_hand_count(self, entity: Entity):
        ...
        box_width = self.ui_renderer.ss(80)
        box_height = self.ui_renderer.ss(100)
        spacing = self.ui_renderer.ss(15)
        ...
        self.ui_renderer.draw_text(
            f"手牌: {hand_size}",
            self.ui_renderer.ui_scale.window_width / 2,
    ...
    def check_hover(self, x: float, y: float):
        ...
        for card_id, (card_x, card_y) in list(self.card_positions.items()):
            ...
            half_width = self.ui_renderer.ss(CONSTANTS.CARD_WIDTH) / 2
            half_height = self.ui_renderer.ss(CONSTANTS.CARD_HEIGHT) / 2
            
            # 考虑悬停偏移
            check_y = card_y
            if id(card) == id(old_hovered):
                check_y += self.ui_renderer.ss(CONSTANTS.CARD_HOVER_OFFSET)
    ...
    def start_rising_animation(self, card: Card, duration: float = 0.5):
        ...
        """
        # 计算起始位置(从屏幕下方)
        start_y_position = -self.ui_renderer.ui_scale.window_height * 0.5
        ...
        if card_id not in self.card_display_posis:
            self.card_display_posis[card_id] = {
                'target_y': self.ui_renderer.sy(60),  # 默认目标位置
            ...
            pos_data['target_y'] = self.ui_renderer.sy(60)  # 确保目标位置正确
    ...
    def _draw_dragged_card(self, card: Card, x: float, y: float):
        ...
        """
        # 缩小版的卡牌尺寸
        drag_width = self.ui_renderer.ss(CONSTANTS.CARD_WIDTH) * 0.6  # 60% 大小
        drag_height = self.ui_renderer.ss(CONSTANTS.CARD_HEIGHT) * 0.6
        ...
        arcade.draw_lrbt_rectangle_outline(
            x - drag_width / 2, x + drag_width / 2,
            y - drag_height / 2, y + drag_height / 2,
            border_color, self.ui_renderer.ss(8)
        )
        ...
        # AP消耗指示器（右上角）
        ap_x = x + drag_width / 2 - self.ui_renderer.ss(10)
        ap_y = y + drag_height / 2 - self.ui_renderer.ss(10)
        ...
        if texture:
            arcade.draw_texture_rect(
                texture,
                arcade.XYWH(
                    ap_x - self.ui_renderer.ss(8),
                    ap_y - self.ui_renderer.ss(8),
                    self.ui_renderer.ss(16),
                    self.ui_renderer.ss(16)
                )
            )
        else:
            arcade.draw_circle_filled(ap_x, ap_y, self.ui_renderer.ss(6), arcade.color.BLUE)
            arcade.draw_circle_outline(ap_x, ap_y, self.ui_renderer.ss(6), arcade.color.WHITE, self.ui_renderer.ss(2))
